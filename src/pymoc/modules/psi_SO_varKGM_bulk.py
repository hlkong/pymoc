...
class Psi_SO_VarKGM_Bulk(object):
  r"""
  ...
  def __init__(
      ...
      K0=2000.,    # reference, effective GM coefficient (in)
      n=1,       # power in the formula of the variable KGM (in)
  ...
  def calc_GM(self):
    ...
    if self.c is not None:
      temp = make_func(
          self.K0 * (D/self.D0)**(self.n-1) * (self.z/dy_atz) * self.L * toptaper * bottaper, self.z,
          'psiGM'
      )
      N2 = self.calc_N2()
      ...
      temp = self.K0 * (D/self.D0)**(self.n-1) * np.maximum(
