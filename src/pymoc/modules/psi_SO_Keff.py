...
class Psi_SO_Keff(object):
  ...
  K = K0*(D/D0)^{n-1} * [1+K1*(tau/tau0)]

  Preliminary fit: K0=600, K1=1.4. These yield a similar K as diagnosed from
  ...
  def __init__(
      ...
      K0=600.,    # reference transient GM coefficient (in)
      n=1,       # power in the formula of the transient KGM (in)
      D0=794.,   # reference pycnoline depth; same value as in Kong (2020) (in)
      K1=1.4,  # sensitivity of stationary eddy stretching factor to wind stress (in)
  ...
  def calc_GM(self):
    ...
    if self.c is not None: # Ferrari et al. (2010) scheme
      temp = make_func(
          (self.K0 * (D/self.D0)**(self.n-1) * (1+self.K1*self.taumax/self.tau0)) \
      ...
      temp = (self.K0 * (D/self.D0)**(self.n-1) * (1+self.K1*self.taumax/self.tau0)) \
