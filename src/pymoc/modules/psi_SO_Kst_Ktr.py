...
class Psi_SO_Kst_Ktr(object):
  ...
  ---- 2/22/2021, Hailu Kong
  Parameterize the SO GM diffusivity as a combination of stationary + transient
  diffusivities, where:
  (1) the transient diffusivity remains to take the form of Ktr = K0*(D/D0)^{n-1}
      with a re-tuned K0 = 500 m^2/s (corresponding to the transient diffusivity
      from shoebox reference run: sb2GnMap12tp2KHTH100)
  (2) the stationary diffusivity is a linear function of wind stress:
      Kst = K1 + K2*tau, where K1 = 900 & K2 = 3000 are the fitted parameters 
      based on shoebox simulations: sb2GnMap12KHTH100
  """
  def __init__(
      self,
      ...
      K0=500.,    # reference transient GM coefficient (in)
      n=1,       # power in the formula of the transient KGM (in)
      D0=794.,   # reference pycnoline depth; same value as in Kong (2020)
      K1=900.,  # stationary GM diffusivity at 0 wind stress (in)
      K2=3000.,  # sensitivity of stationary GM diffusivity to wind stress (in)
  ...
  def calc_GM(self):
    ...
    if self.c is not None:
      temp = make_func(
          (self.K0 * (D/self.D0)**(self.n-1) + self.K1 + self.K2*self.tau0) \
      ...
      temp = (self.K0 * (D/self.D0)**(self.n-1) + self.K1 + self.K2*self.tau0) \
