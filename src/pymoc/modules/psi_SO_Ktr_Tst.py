...
class Psi_SO_Ktr_Tst(object):
  ...
  ------- Modification from psi_SO_Kst_Ktr.py ---------
  ---- 7/27/2021, Hailu Kong
  Inherit the way transient eddies-induced MOC is parameterized
    while re-set the stationary eddies-induced MOC as a linear function
    only of wind stress: K1 + K2*\tau/\tau_{text}, where the initial fit
    is: K1 = 1.75, K2 = 2.33, based on the decomposition of SOMOC at 40S in the GCM
  """
  def __init__(
      self,
      ...
      tau_ref = 0.2, # reference wind stress (float, in)
      f=1.2e-4,    # Coriolis parameter (in)
      rho=1030,    # Density of sea water (in)
      L=1e7,    # Zonal length of the ACC (in)
      K0=580.,    # reference transient GM coefficient (in)
      n=1,       # power in the formula of the transient KGM (in)
      D0=818.,   # reference pycnoline depth; same value as in Kong (2020)
      K1=1.75,  # stationary eddy-induced MOC at 0 wind stress (in)
      K2=2.33,  # sensitivity of stationary eddy-induced MOC to normalized wind stress (in)
  ...
  def calc_GM(self):
    ...
    if self.c is not None:
      temp = make_func(
          (self.K0 * (D/self.D0)**(self.n-1) * (self.z/dy_atz) * self.L \
           - 1e6*(self.K1 + self.K2 * self.tau0/self.tau_ref)) \
          * toptaper * bottaper, self.z, 'psiGM'
      )
      N2 = self.calc_N2()
      ...
      temp = (self.K0 * (D/self.D0)**(self.n-1) \
              *np.maximum( self.z / dy_atz, -self.smax) * self.L \
              - 1e6*(self.K1 + self.K2 * self.tau0/self.tau_ref)) \
             * toptaper * bottaper
