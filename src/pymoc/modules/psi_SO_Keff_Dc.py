...
class Psi_SO_Keff_Dc(object):
  ...
  ----------------- Modification from psi_SO_Keff --------------
  Use the cut-off version of D in computing the parameterized eddy-induced SOMOC.
  
  Specifically, 
  \psi_\mathrm{eddy} = K_1*(1+\alpha*\tau/\tau_0)*(D_c/D_0)^{n-1}*D(b)/L_y*L_x,
  where D_c is the pycnocline depth cut off at the bottom buoyancy of the AMOC,
  which is interpolated based on the buoyancy profile (b-z) and AMOC profile (psi-z)
  in the northern sinking region. Importantly, the cut-off buoyancy varies with 
  time and is always the bottom of the AMOC, to ensure the smoothness of D timeseries.
  
  In this script, the cut-off is passed in as an argument, which is computed outside
  of the scope. 
  """
  def __init__(
      self,
      ...
      D0=818.,   # reference pycnoline depth, as diagnosed from the GCM run b/t 40S & 40N (in)
      K1=1.1,  # sensitivity of stationary eddy stretching factor to wind stress (in)
      Dc=300., # the cut-off D passed to this object. To be used in calc_GM (in)
  ...
  def calc_GM(self):
    ...
    if self.c is not None: # Ferrari et al. (2010) scheme
      temp = make_func(
          (self.K0 * (self.Dc/self.D0)**(self.n-1) * (1+self.K1*self.taumax/self.tau0)) \
      ...
      temp = (self.K0 * (self.Dc/self.D0)**(self.n-1) * (1+self.K1*self.taumax/self.tau0)) \
  ...
  def update(self, b=None, bs=None, Dc=None):
    r"""
    Update the vertical buoyancy profile, the surface buoyancy, and the cut-off 
    pynocline depth, based on changes in the adjoining basin and/or in the surface 
    boundary conditions.
    ...
    bs : float, function, or ndarray
         ...
         or an array or function in y. Units: m/s\ :sup:`2`
    Dc : float
         Cut-off version of pycnocline depth, considering only the levels within the AMOC range
    """
