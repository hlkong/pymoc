...
class Psi_SO_VarKGM_Local(object):
  r"""
  ...
  Update: 1/7/2021, Hailu Kong
  
  Replace the constant GM diffusivity by the one proposed by J11
  
  Specifically, kGM = K*(z/z0)^(n-1), where K = 1000 is the reference kGM, 
  z is the array of isopycnal depths for each layer, 
  z0 is the array for isopycnal depths for each layer under the control wind stress,
  and n is a tunable power parameter
  
  z0 is taken from the reference wind stress simulation, which uses the original 
  psi_SO.py module
  ...
  def __init__(
      ...
      n=1,      # the power n in Psi_GM = KGM*s^n (in): e.g., n=1 denotes a constant kGM, 
                # n=2 is similar to Visbeck et al. (1997) or TMEKE
  ...
  def calc_GM(self):
    ...
    if self.c is not None:
      temp = make_func(
          self.KGM * (np.abs(self.z / dy_atz))**self.n * np.sign(self.z/dy_atz) * self.L * toptaper * bottaper, self.z,
          'psiGM'
      )
      N2 = self.calc_N2()
      ...
      temp = self.KGM * (np.abs(np.maximum(
          self.z / dy_atz, -self.smax
      )))**self.n * np.sign(np.maximum(
          self.z / dy_atz, -self.smax
