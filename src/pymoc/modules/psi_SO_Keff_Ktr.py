...
class Psi_SO_Keff_Ktr(object):
  ...
  ------ Modification from the origional pymoc Psi_SO class -----
  -- 1/19/2021, Hailu Kong
  Replaced the constant KGM by a slope-dependent formulation, 
  as in Jones et al. (2011): KGM ~ K0 * (D/D0)^{n-1}, 
  where K0 is a reference, effective (i.e. combining both transient 
  and stationary) diffusivity, D the pycnoline depth in the basin
  (following note 11 in Gnanadesikan (1999)), D0 a reference
  pycnocline depth, and n a specified power (n = 1 to retrieve a constant KGM
  and n = 2 to retrieve the Visbeck (1999) - type of parameterization).
  - Note that different from Jones et al. (2011), we only parameterize KGM to 
    be dependent on D & n; the isopycnal slope s, used to compute the GM MOC,
    remains to be locally determined, as originally implemented in psi_SO.py
  
  ------- Further modification from psi_SO_varKGM_bulk.py ---------
  ---- 3/2/2021, Hailu Kong
  Parameterize the SO GM diffusivity as an effective diffusivity, where
  the role of stationary eddies is treated as a stretching of transient eddies,
  as is discussed in Kong & Jansen (2021) Appendix C. Specifically,
  
  K = K1*(D/D0)^{n-1} * [1+\alpha*(tau/tau0)]

  Preliminary fit: K1=600, alpha=1.4. These yield a similar K as diagnosed from
  b019_n8 run, which gives the cloeset pycnocline depth.


  -------------- Update from psi_SO_Keff.py ---------------
  ------------ 8/23/2021, Hailu Kong ----------------------
  1. Introduce a modified version of the stretching factor: c1 + c2 * tau0/tau_ref
  2. Introduce this stretching factor also into the expression of the transient
       diffusivity part, such that the total expression of Keff becomes:
       
       Keff = K2*(D/D0)^{n-1} * [c1+c2*(tau0/tau_ref)]^{(n-1)/2} * [c1+c2*(tau0/tau_ref)]
  ...
  def __init__(
      ...
      K2=295.,    # reference transient GM coefficient (in)
      n=1,       # power in the formula of the transient KGM (in)
      D0=818.,   # reference pycnoline depth; same value as in Kong (2020) (in)
      c1=1.59,  # the stretching facter under 0 wind stress
      c2=2.89,  # sensitivity of stationary eddy stretching factor to wind stress (in)
    ...
    self.taumax = np.max(tau) # peak wind stress over the SO
    self.tau0 = 0.2 # reference peak wind stress over the SO
  ...
  def calc_GM(self):
    ...
    D = -np.sum((self.b(self.z[1:]) - self.b(self.z[0])) * self.z[1:] * \
        (self.z[1:] - self.z[:-1])) / np.sum((self.b(self.z[1:]) - self.b(self.z[0])) \
        * (self.z[1:]-self.z[:-1])) # pycnocline depth D in the basin; positive value
    if self.c is not None: # Ferrari et al. (2010) scheme
      temp = make_func(
          (self.K2 * (D/self.D0)**(self.n-1) \
          * (self.c1+self.c2*self.taumax/self.tau0)**((self.n+1)/2)) \
          * (self.z/dy_atz) * self.L * toptaper * bottaper, self.z,
          'psiGM'
      )
      N2 = self.calc_N2()
      ...
      temp = (self.K2 * (D/self.D0)**(self.n-1) \
             * (self.c1+self.c2*self.taumax/self.tau0)**((self.n+1)/2)) \
             * np.maximum( self.z / dy_atz, -self.smax) \
